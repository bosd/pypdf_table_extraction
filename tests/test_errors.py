...
def test_lattice_unknown_backend(foo_pdf):
    ...
    with pytest.raises(NotImplementedError, match=message):
        tables = camelot.read_pdf(foo_pdf, flavor="lattice", backend="mupdf", use_fallback=False)
...
def test_lattice_no_convert_method(foo_pdf):
    ...
    with pytest.raises(NotImplementedError, match=message):
        camelot.read_pdf(foo_pdf, flavor="lattice", backend=ConversionBackend(), use_fallback=False)
